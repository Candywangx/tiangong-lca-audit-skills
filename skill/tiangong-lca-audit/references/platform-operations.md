...
审核判断与平台操作是两条独立流程。任何平台写入都必须在人工确认之后执行；先形成并由人工确认审核结论及待写内容，再执行平台动作。生成平台退回意见不等于授权保存草稿，保存草稿也不等于授权提交、通过或驳回。
...
1. 读取并确认已验证的 `platform_comment`，核对其对应本轮审核对象且 `valid=true`。
2. 确认其中的 opinion 与 routed findings 完整、可定位、可执行；不得在操作阶段按 `severity` 重新筛选、增删或改写意见。
3. 由人工确认最终平台文字和待执行操作清单，到此停止。
